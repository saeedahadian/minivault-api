...
def generate(prompt: str, url: str):
    ...
    with console.status("[bold green]Sending request..."):
        try:
            ...
            if 'usage' in data:
                usage = data['usage']
                console.print(f"[dim]Usage: {usage['prompt_tokens']} prompt + {usage['completion_tokens']} completion = {usage['total_tokens']} total tokens[/dim]")
    ...
    async def stream_tokens():
        async with httpx.AsyncClient() as client:
            try:
                ...
                usage_info = None
                async with client.stream('POST', f"{url}/generate", json={"prompt": prompt, "stream": True}) as response:
                ...
                console.print("\n")
                if usage_info:
                    console.print(f"[dim]Usage: {usage_info['prompt_tokens']} prompt + {usage_info['completion_tokens']} completion = {usage_info['total_tokens']} total tokens[/dim]")
            except Exception as e:
...
def benchmark(count: int, url: str):
    ...
    with console.status("[bold green]Benchmarking...") as status:
        for i in range(count):
            ...
            try:
                response = httpx.post(f"{url}/generate", json={"prompt": f"Benchmark request {i}"})
                response.raise_for_status()
